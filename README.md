...
撞到 429 先停 60s。如果它恢复后立刻又 429（说明是更长周期的额度，不是每分钟限流），就翻倍：60s → 120s → 240s …… 到 `cooldown_max` 为止。**一旦成功，计数清零**，不会因为历史记录被一直惩罚。
...
当前 key 全部处于冷却时，代理会把恢复最早的那个**再试一次**（它可能已经提前恢复了）。还是 429 才把 429 返回给调用者，并带 `Retry-After` 告诉它多久后再来。
...
容器里可以用环境变量 `CLINE_KEYS`（逗号分隔）替代配置文件里的 `keys`。

## 端点

| 端点 | 说明 |
| --- | --- |
| `POST /v1/chat/completions` | 唯一的转发端点 |
| `GET /status` | 每个 key 正在服务哪些模型、在冷却哪些模型、还剩多久（key 已打码） |
| `GET /healthz` | 存活探针 |
